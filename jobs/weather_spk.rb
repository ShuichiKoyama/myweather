...
#  response = http.request(Net::HTTP::Get.new("/data/2.5/weather?id=#{CITY_ID}&units=#{UNITS}&appid=#{API_KEY}"))
  response = http.request(Net::HTTP::Get.new("/data/2.5/weather?q=#{CITY_NAME_s}&units=#{UNITS}&appid=#{API_KEY}"))
...
#  print response.body
  ...
  send_event('weather_spk', { :temp => "#{current_temp} &deg;#{temperature_units}",
